...
export function SearchDashboard() {
  ...
  React.useEffect(() => {
    ...
    const handleNativeWheel = (e: WheelEvent) => {
      ...
      if (e.deltaY > 0) {
        setScrollRow((prev) =>
          Math.min(maxScrollableRowsRef.current, prev + 1),
        );
      } else {
  ...
  const handleScrollDrag = (e: React.MouseEvent) => {
    ...
    const updateScroll = (clientY: number) => {
      ...
      const y = Math.max(
        0,
        Math.min(rect.height - 30, clientY - rect.top - 15),
      );
  ...
  const getIconForTab = (id: string) => {
    ...
    switch (id) {
      ...
      case "Functional Blocks":
        iconName = "Oak_Sign";
        break;
      case "Redstone Blocks":
        iconName = "Redstone_Dust";
        break;
      case "Tools & Utilities":
        iconName = "Iron_Pickaxe";
        break;
      case "Combat":
        iconName = "Golden_Sword";
        break;
      case "Food & Drinks":
        iconName = "Golden_Apple";
        break;
      case "Ingredients":
        iconName = "Iron_Ingot";
        break;
      case "Spawn Eggs":
        iconName = "Pig_Spawn_Egg";
        break;
      case "search":
        iconName = "Compass";
        break;
    ...
    const extension =
      iconName === "Compass" || iconName === "Clock" ? "gif" : "png";
        ...
        <div className="relative flex flex-col items-center w-[392px] transform scale-[0.85] sm:scale-[1.25] md:scale-[1.5] lg:scale-[1.75] xl:scale-[2] origin-top mb-[-40px] sm:mb-[100px] md:mb-[200px] lg:mb-[300px] xl:mb-[400px]">
          ...
          <div className="mc-window w-[392px] h-[272px] flex flex-col items-center pt-[16px] pb-[16px] z-20">
            <div className="flex justify-between items-end w-[356px] h-[24px] mb-[12px]">
              <h2
                className="text-[#373737] font-bold text-[16px] leading-none select-none tracking-tight animate-pulse"
            ...
            <div className="relative w-[356px] h-[180px] flex gap-[4px] animate-pulse">
              ...
              <div className="mc-scrollbar flex flex-col items-center"></div>
            </div>
          </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4 overflow-x-hidden w-full">
      <div
        ref={gridRef}
        className="relative flex flex-col items-center w-[392px] transform scale-[0.85] sm:scale-[1.25] md:scale-[1.5] lg:scale-[1.75] xl:scale-[2] origin-top mb-[-40px] sm:mb-[100px] md:mb-[200px] lg:mb-[300px] xl:mb-[400px]"
      >
        {/* Top Tabs */}
        <div className="flex w-[392px] items-end z-10 relative">
          {topTabs.map((tabId) => (
            <button
              ...
              className={cn(
                "mc-tab-top flex items-center justify-center cursor-pointer",
        ...
        <div className="mc-window w-[392px] h-[272px] flex flex-col items-center pt-[16px] pb-[16px] z-20">
          <div className="flex justify-between items-end w-[356px] h-[24px] mb-[12px]">
            <h2
              className="text-[#373737] font-bold text-[16px] leading-none select-none tracking-tight"
              style={{ fontFamily: "monospace" }}
            >
              {currentChamberName}
            </h2>
        ...
        <div className="flex w-[392px] items-start z-10 relative">
          {bottomTabs.map((tabId) => (
            <button
              ...
              onClick={() => setActiveTab(tabId)}
              className={cn(
                "mc-tab-bottom flex items-center justify-center cursor-pointer",
                activeTab === tabId && "active",
              )}
              title={tabId}
            >
